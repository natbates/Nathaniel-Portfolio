...
class CanvAscii {
  constructor(
    { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, maxRotation = 0.5 },
    containerElem,
    width,
    height
  ) {
    ...
    // maximum rotation in radians applied to mesh
    this.maxRotation = maxRotation;

    this.camera = new THREE.PerspectiveCamera(45, this.width / this.height, 1, 1000);
    this.camera.position.z = 30;

    this.rotateOnMobile = true;
    this.mobileRotateWidthThreshold = 768;
    this.basePlaneW = 0;
    this.basePlaneH = 0;
  ...
  shouldRotatePortrait() {
    if (!this.rotateOnMobile) return false;
    const isPortrait = this.height > this.width;
    const isSmallScreen = this.width <= this.mobileRotateWidthThreshold;
    return isPortrait && isSmallScreen;
  }

  updateMeshOrientation() {
    if (!this.mesh) return;
    const rotate = this.shouldRotatePortrait();
    this.isPortraitRotated = rotate;
    this.mesh.rotation.z = rotate ? Math.PI / 2 : 0;
  }

  fitCameraToPlane({ margin = 1.15 } = {}) {
    if (!this.camera || !this.basePlaneW || !this.basePlaneH) return;
    if (!this.width || !this.height) return;

    const fov = (this.camera.fov * Math.PI) / 180;
    const aspect = this.width / this.height;

    const effectivePlaneW = this.isPortraitRotated ? this.basePlaneH : this.basePlaneW;
    const effectivePlaneH = this.isPortraitRotated ? this.basePlaneW : this.basePlaneH;

    const distForHeight = effectivePlaneH / (2 * Math.tan(fov / 2));
    const distForWidth = effectivePlaneW / (2 * Math.tan(fov / 2) * aspect);
  ...
  setMesh() {
    ...
    this.basePlaneW = planeW;
    this.basePlaneH = planeH;
  ...
  updateRotation() {
    // map mouse position to target rotation, then clamp within maxRotation
    let x = Math.map(this.mouse.y, 0, this.height, 0.5, -0.5);
    let y = Math.map(this.mouse.x, 0, this.width, -0.5, 0.5);

    if (this.maxRotation !== undefined && this.maxRotation !== null) {
      x = Math.clamp(x, -this.maxRotation, this.maxRotation);
      y = Math.clamp(y, -this.maxRotation, this.maxRotation);
    }
  ...
  useEffect(() => {
    ...
    const createAndInit = async (container, w, h) => {
      const instance = new CanvAscii(
        { text, asciiFontSize, textFontSize, textColor, planeBaseHeight, enableWaves, maxRotation },
        container,
        w,
        h
      );
