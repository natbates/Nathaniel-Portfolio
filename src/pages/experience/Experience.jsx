...
import messages from "./experience.messages";
...
export default function Experience() {
  ...
  const years = useMemo(() => getAvailableYears(experienceData), []);

  const filtered = useMemo(
    () => getSortedFilteredItems(experienceData, { search, sort, year }),
    [search, sort, year]
  );

  const { pagedItems, totalPages, currentPage } = useMemo(
    () => paginateItems(filtered, page, ITEMS_PER_PAGE),
    [filtered, page]
  );
